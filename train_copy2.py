...
from algorithm.tdmpc_copy2 import TDMPC
from algorithm.helper_copy2 import Episode, get_demos, ReplayBuffer, Memory, my_get_demos
...
def train(cfg: dict):
    ...
    # Load demonstrations
    episode_0 = my_get_demos(cfg)[0]
    memory = Memory(episode_0)
    if cfg.get("demos", 0) > 0:
        ...
        memory.save_value('/home/anyone/otp/outputs/memory', 'test')
        if cfg.get("demo_schedule", 0) != 0:
            agent.init_bc(demo_buffer)
        else:
            agent.init_bc(buffer)
        # agent.init_bc(demo_buffer if cfg.get("demo_schedule", 0) != 0 else buffer)
        print(colored("\nPhase 2: seeding", "green", attrs=["bold"]))
    eval_rew, eval_succ = evaluate(env, agent, cfg, 0, 0, False)
    print(f'bc_score: {eval_succ}')
    print(f'bc_reward: {eval_rew}')
    # Run training
    ...
    for step in range(0, cfg.train_steps + cfg.episode_length, cfg.episode_length):
        ...
        # Collect trajectory
        obs = env.reset()
        init_embeding = memory.encoder_obs(obs)
        ...
        while not episode.done:
            ...
            memory.update(init_embeding, memory.encoder_obs(obs), i)
            if cfg.DEBUG and step==0: t.save_rgb_array_as_jpg(os.getcwd()+'/debug', str(i), obs[0:3])
        assert (
            len(episode) == cfg.episode_length
        ), f"Episode length {len(episode)} != {cfg.episode_length}"
        buffer += episode

        if episode_idx%50==0: memory.save_value('/home/anyone/otp/outputs/memory', str(episode_idx))
        ...
        if step >= cfg.seed_steps:
            ...
            if step == cfg.seed_steps:
                for i in tqdm(range(num_updates), 'Pretraining model'):
